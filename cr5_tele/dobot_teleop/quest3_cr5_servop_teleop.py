...
class Quest3CR5ServoPTeleop(Node):
    ...
    def finish_enable_teleop(self, robot_pose: list):
        with self.state_lock:
            ...
            # GetPose is asynchronous.  If the operator moves the controller
            # while we wait for the robot pose, using the old request-time VR
            # pose turns that delay into the first ServoP target jump.
            self.vr_start_pose = deepcopy(
                self.latest_vr_pose or self.requested_vr_start_pose
            )
