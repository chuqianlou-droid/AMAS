...
def make_policy_source(args: argparse.Namespace) -> Any:
    ...
    client_source = workspace / "openpi" / "packages" / "openpi-client" / "src"
...
def main() -> None:
    ...
    if tool_offset_cfg.effective():
        print(
            ...
            "BEFORE sending to ServoP.  No 16 cm double-offset."
        )
    ...
    print(f"action={args.action_format}, delta_frame={args.delta_frame}, rate={args.command_rate:.1f} Hz")
    print(
        "Safety: "
    ...
    if not args.execute:
        print("DRY RUN: ServoP, gripper, and Dobot network commands are disabled. Pass --execute to arm motion.")
    ...
    try:
        if args.execute:
            ...
            if args.enable_gripper:
                gripper = PgeModbusGripper(
                    client, args.gripper_slave_id, args.gripper_baud, args.gripper_parity,
                    args.gripper_data_bit, args.gripper_stop_bit, args.gripper_force,
                    args.gripper_speed, args.gripper_open_position, args.gripper_close_position,
                    args.gripper_trigger_threshold, args.gripper_init_timeout,
                )
        ...
        while args.max_actions == 0 or sent < args.max_actions:
            ...
            if not action_queue:
                ...
                print(f"[PI0-CR5A] raw actions shape = {raw_actions.shape}")
                ...
                if action_kind == "aloha_14d":
                    print("[PI0-CR5A] detected action format: aloha_14d")
                    print("[PI0-CR5A] This is inference-only. ALOHA 14D action cannot be executed on CR5A.")
                    break
                action_queue = deque(
                    np.array(action, dtype=float) for action in raw_actions[: max(args.open_loop_horizon, 1)]
                )

            action = action_queue.popleft()
            model_target, gripper_trigger = adapter.adapt(action, governor.current_target())

            # ── Convert gripper_center → TCP for ServoP ──────────────────
            # The model outputs targets in gripper_center space (same frame as
            # training data).  Dobot ServoP expects TCP targets only.
