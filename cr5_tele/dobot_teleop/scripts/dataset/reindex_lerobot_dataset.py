...
This is useful when a LeRobot dataset has metadata for episodes that are no
longer present on disk. The script copies only existing parquet episodes to a
new dataset directory and rewrites episode indices so they are contiguous.
"""
...
def _fix_huggingface_metadata(table: pa.Table) -> pa.Table:
    """Make older HF parquet metadata compatible with current `datasets`.

    Some locally written files use `_type: List` for fixed-length vector
    features. Newer `datasets` expects `_type: Sequence`. Keeping the metadata
    is important because it preserves image decoding for embedded image columns.
    """
...
def _aggregate_stats(episode_stats: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for key in keys:
        ...
        for row in episode_stats:
            ...
            count_value = float(count.reshape(-1)[0])
        ...
        mean = (means_arr * weights).sum(axis=0) / total
        second_moment = ((stds_arr**2 + means_arr**2) * weights).sum(axis=0) / total
...
def reindex_dataset(input_dir: Path, output_dir: Path, *, overwrite: bool = False) -> None:
    ...
    existing_old_indices: list[int] = []
    missing_old_indices: list[int] = []
...
def main() -> None:
    ...
    parser.add_argument("--input-dir", type=Path, required=True, help="Existing LeRobot dataset directory")
    parser.add_argument("--output-dir", type=Path, required=True, help="New reindexed dataset directory")
