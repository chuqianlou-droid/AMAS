...
"""
Direct Quest3 UDP -> Dobot ServoJ / ServoP teleoperation (no ROS).
...
# ---------------------------------------------------------------------------
# Each tuple is (lower, upper) in degrees.
CR5_JOINT_LIMITS = [
    (-360.0, 360.0),  # J1
    (-360.0, 360.0),  # J2
...
def check_joint_limits(joints):
    """Return (ok, message).  ok=False means at least one joint is outside the
                    ...
                    if not args.dry_run:
                        if args.servo_mode == "joint":
                            ...
                            try:
                                ...
                                # Don't crash — skip and continue teleop
                                continue
